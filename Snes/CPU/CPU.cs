...
namespace Snes
{
    partial class CPU : CPUCore, IPPUCounter, IProcessor, IMMIO
    {
        ...
        private byte mmio_r4212()
        {
            ...
            if (PPUCounter.vcounter() >= vs && PPUCounter.vcounter() <= (vs + 2)) r |= 0x01;  //auto joypad polling
            if (PPUCounter.hcounter() <= 2 || PPUCounter.hcounter() >= 1096) r |= 0x40;  //hblank
            if (PPUCounter.vcounter() >= vs) r |= 0x80;  //vblank
            return r;
        }
